...
export default {
  ...
  methods: {
    ...
    beforeTabLeave(activeName, oldActionName) {
      // console.log('即将离开的标签页名字是'+ oldActionName)
      // console.log('即将进入的标签页名字是'+ activeName)
      // return false
      if (oldActionName === '0' && this.addForm.goods_cat.length !== 3) {
    ...
    handleRemove(file) {
      ...
      // 3.调用数组中的 splice 方法，把图片信息对象，从 pics 数组中移出
    ...
    add(){
      //console.log(this.addForm)
      this.$refs.addFormRef.validate(async valid => {if(!valid){return this.$message.error('请填写必要的表单！')}
      // 执行添加的业务逻辑
